        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                	...
                	sb.AppendFormat(String.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/INVENIO/_The" +
                                "me_DarkKnight.css?{0}\', type: \'text/css\', rel: \'stylesheet\' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework=\'4.6\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'XZEDI_SERV" +
                        "ICES_REPORTS|{0}\';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            if (!(String.IsNullOrEmpty(callback)))
            	...
            	if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("INVENIO=typeof INVENIO==\'undefined\'?{{}}:INVENIO;INVENIO.{0}=", instance);
                }
        }
